...
void printStat(){
	...
	if((outRes & XML)!=0) {
		...
		fprintf(xml,"\t<prm ");
		fprintf(xml,"kernel=\"%s\" kernelSigma=%.0f kernelShift=%.0f kernelNS=%.0f ",getKernelType(), kernelSigma,kernelShift,kernelNS*100);
		fprintf(xml,"wSize=%i wStep=%i flankSize=%i noiseLevel=%.2f ",wSize, wStep,flankSize,noiseLevel);
		fprintf(xml,"nShuffle=%i maxShuffle=%i threshold=%i ",nShuffle, maxShuffle,threshold);
		fprintf(xml,"stepSize=%i bpType=%i ",stepSize, bpType);
		fprintf(xml,"NAFlag=%i maxNA=%.1f maxZero=%.1f ",NAFlag, maxNA0,maxZero0);
		fprintf(xml,"intervFlag=%x strandFg=%i complFg=%i ",intervFlag0, strandFg0,complFg);
