...
int checkFilter(char * type){
	if(biotypes ==0) return 0;
	for(int i=0; type_filter[i]; i++){
		if(keyCmp(type,type_filter[i])==0){
			return 0;
		}
	}
	return 1;
...
void parseGTF(){
	...
	for(int iLine=0; (inputString=fgets(bb,SG_BUFEXT,fin))!=0; iLine++){
		if(iLine % 10000 ==0) verb("%i\r", iLine);
		inputString=trim(inputString);
		if(*inputString=='#') continue;


		if(record.parse(inputString)){
			if(record.level > pgLevel || checkFilter(record.genetype)) {continue;}
//record.print(fgene);
...
//	debugFg=DEBUG_LOG|DEBUG_PRINT;
	...
	if(biotypes){
		char *s=0;
		char *bb=biotypes;
		for(int i=0;(s=strtok(bb," ,;\t"))!=0;i++){
			bb=0;
			type_filter[i]=strdup(s);
			type_filter[i+1]=0;
		}
	}
	fin=xopen(argv[1], "r");
